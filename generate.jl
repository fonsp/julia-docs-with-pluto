...
begin
    ...
    Pkg.add([
        Pkg.PackageSpec(name="Pluto", version="0.14.1"),
...
"functions.md"
"getting-started.md"
"handling-operating-system-variation.md"
...
"modules.md"
"multi-threading.md"
"networking-and-streams.md"
"noteworthy-differences.md"
"parallel-computing.md"
"performance-tips.md"
"profile.md"
"running-external-programs.md"
"stacktraces.md"
"strings.md"
"style-guide.md"
"types.md"
...
downloadlink(filename) = "https://raw.githubusercontent.com/JuliaLang/julia/v1.6.0/doc/src/manual/$(filename)"
...
@bind input_md TextField((60,20); default="""
# Methods
...
parsed_md = Markdown.parse(input_md);

# ╔═╡ 714e97eb-52e8-4230-b67e-760246d267e3
parsed_md.content

# ╔═╡ 5fa8b74c-59c7-443e-b102-e8ba238e25a6
...
# ╔═╡ 5559a9e9-61f0-4c96-97cd-b6c07c866425
markdown_cell(x) = Pluto.Cell(code=markdown_cell_code(x), code_folded=true)

# ╔═╡ 580fdc42-7bd1-4cd7-8e8c-ea4b1da5278d
function split_to_cells(input_code_block::String)
	splits = split(input_code_block, r"(\n)*julia> "; keepempty=false)
...
to_pluto_cells(m) = [markdown_cell(m)]

# ╔═╡ 0d0d85fe-f1e0-450a-9b48-5fa4e9d03aa7
function doc_to_notebook(contents)
	parsed_md = Markdown.parse(contents)
...
# ╔═╡ 20794d37-301b-4031-a514-4fdbcdc3b6d7
nb = Pluto.Notebook(union(to_pluto_cells.(parsed_md.content)...))

# ╔═╡ 3a7ccfda-7152-48c1-a8c2-da80d63470c0
PlutoUI.DownloadButton(sprint(Pluto.save_notebook, nb), "docs.pluto.jl")

# ╔═╡ 2062adf2-f2d8-4854-82c0-1354e4fd9e23
Pluto.save_notebook |> methods

# ╔═╡ 3786b45f-2ac9-4335-96ed-15527d068ce3
code_paragraphs = filter(x -> x isa Markdown.Code, parsed_md.content)

# ╔═╡ cfedd4d4-6d90-4966-8c9a-fdc73c3c6344
[c.language for c in code_paragraphs] |> Set

# ╔═╡ 7e7ccc03-22e0-4b09-843a-0878cc5f58f2
Markdown.MD.(parsed_md.content)

# ╔═╡ a4f0efce-a7fa-4790-8811-5c8da43a6e86
md"""
# Parsing code blocks
"""

# ╔═╡ 503934ec-9e10-11eb-17c8-513ce86aceb9
@bind input_code_block TextField((60,20); default="""
julia> 3//5 + 1
8//5
...
# ╔═╡ b7a7f25f-09cb-4c23-b60a-3208caccc000
function results(codes)
	m = Module()
	
	map(codes) do c
		try
			Core.eval(m, Meta.parse(c))
		catch e
...
# ╔═╡ f588c2da-a0b7-4546-ae28-f169691045a7
codes = map(splits) do code
	expr, ind = Meta.parse(code, 1; greedy=true, raise=false)
	code[1:prevind(code, ind)] |> strip
end

# ╔═╡ 4ffbf073-8bc8-4ba7-832c-9a55b5fcf07c
results(codes)

# ╔═╡ 8c4ab453-8618-4b3a-881c-9a48030f22c4
repr.([MIME"text/plain"()], results(codes))

# ╔═╡ 01628046-e038-4b74-8a67-751fd531bb20
expected_results = map(splits) do code
	expr, ind = Meta.parse(code, 1; greedy=true, raise=false)
	code[ind:end] |> strip
end

# ╔═╡ d1af1d2d-8d46-4f5a-8866-610b2cd6d25c
expected_results
